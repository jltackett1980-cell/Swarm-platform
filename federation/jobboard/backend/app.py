...
app = Flask(__name__)
CORS(app)
SECRET_KEY = 'swarm-jobboard-secret'
...
def health():
    return jsonify({'status': 'healthy', 'app': 'HireLocal', 'timestamp': datetime.now().isoformat()})
    ...
    try:
        ...
        with get_db() as db:
            cursor = db.execute(
                ...
                (data.get('title'), data.get('company'), data.get('location'), data.get('salary'), data.get('type'), data.get('posted'),)
            )
            db.commit()
...
if __name__ == '__main__':
    init_db()
    print(f'Starting HireLocal on http://localhost:5000')
