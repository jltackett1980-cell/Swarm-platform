...
app = Flask(__name__)
CORS(app)
SECRET_KEY = 'swarm-gym-secret-gen3'
...
def health():
    return jsonify({"status": "healthy", "app": "GymOS", "generation": 3, "timestamp": datetime.now().isoformat()})
...
def stats():
    try:
        ...
        return jsonify({"total": total, "active": active, "today": recent, "domain": "gym", "generation": 3})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
