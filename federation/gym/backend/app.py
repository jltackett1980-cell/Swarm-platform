...
app = Flask(__name__)
CORS(app)
SECRET_KEY = 'swarm-gym-secret-gen4'
...
def health():
    return jsonify({"status": "healthy", "app": "GymOS", "generation": 4, "timestamp": datetime.now().isoformat()})
...
def create_member():
    try:
        ...
        with get_db() as db:
            db.execute(
                "INSERT INTO members (name, email, phone, plan, start, expires) VALUES (?, ?, ?, ?, ?, ?)",
...
def update_member(item_id):
    try:
        ...
        with get_db() as db:
            db.execute(
                "UPDATE members SET name=?, email=?, phone=?, plan=?, start=?, expires=?, updated_at=? WHERE id=?",
...
def get_members_v1():
    ...
    try:
        ...
        with get_db() as db:
            ...
            rows = db.execute("SELECT * FROM members ORDER BY created_at DESC LIMIT ? OFFSET ?", (limit, offset)).fetchall()
        return jsonify({
...
def stats():
    try:
        ...
        return jsonify({"total": total, "active": active, "today": recent, "domain": "gym", "generation": 4})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
