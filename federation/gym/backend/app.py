...
def health():
    return jsonify({'status': 'healthy', 'app': 'GymOS', 'timestamp': datetime.now().isoformat()})
    ...
    try:
        ...
        with get_db() as db:
            cursor = db.execute(
                ...
                (data.get('name'), data.get('email'), data.get('phone'), data.get('plan'), data.get('start'), data.get('expires'),)
            )
            db.commit()
...
if __name__ == '__main__':
    init_db()
    print(f'Starting GymOS on http://localhost:5000')
