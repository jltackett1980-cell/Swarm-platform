...
app = Flask(__name__)
CORS(app)
SECRET_KEY = 'swarm-law-secret-gen4'
...
def health():
    return jsonify({"status": "healthy", "app": "LegalDesk", "generation": 4, "timestamp": datetime.now().isoformat()})
...
def stats():
    try:
        ...
        return jsonify({"total": total, "active": active, "today": recent, "domain": "law", "generation": 4})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
