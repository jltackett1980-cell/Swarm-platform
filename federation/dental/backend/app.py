...
app = Flask(__name__)
CORS(app)
SECRET_KEY = 'swarm-dental-secret'
...
def health():
    return jsonify({'status': 'healthy', 'app': 'DentalPro', 'timestamp': datetime.now().isoformat()})
    ...
    try:
        ...
        with get_db() as db:
            cursor = db.execute(
                ...
                (data.get('name'), data.get('phone'), data.get('last_visit'), data.get('next_visit'), data.get('treatment'), data.get('insurance'),)
            )
            db.commit()
...
def update_patients(item_id):
    try:
        data = request.get_json()
        sets = ", ".join([f"{k}=?" for k in data.keys()])
        vals = list(data.values()) + [item_id]
        with get_db() as db:
            db.execute(f"UPDATE patients SET {sets} WHERE id=?", vals)
            db.commit()
...
def stats():
    try:
        ...
        return jsonify({'total': count, 'domain': 'dental'})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
if __name__ == '__main__':
    init_db()
    print(f'Starting DentalPro on http://localhost:5000')
