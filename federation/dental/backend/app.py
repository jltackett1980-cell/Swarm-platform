...
app = Flask(__name__)
CORS(app)
SECRET_KEY = 'swarm-dental-secret-gen5'
...
def health():
    return jsonify({"status": "healthy", "app": "DentalPro", "generation": 5, "timestamp": datetime.now().isoformat()})
...
def get_patients_v1():
    ...
    try:
        ...
        with get_db() as db:
            ...
            rows = db.execute("SELECT * FROM patients ORDER BY created_at DESC LIMIT ? OFFSET ?", (limit, offset)).fetchall()
        return jsonify({
...
def stats():
    try:
        ...
        return jsonify({"total": total, "active": active, "today": recent, "domain": "dental", "generation": 5})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
