...
app = Flask(__name__)
CORS(app)
SECRET_KEY = 'swarm-church-secret-gen6'
...
def health():
    return jsonify({"status": "healthy", "app": "ChurchDesk", "generation": 6, "timestamp": datetime.now().isoformat()})
...
def stats():
    try:
        ...
        return jsonify({"total": total, "active": active, "today": recent, "domain": "church", "generation": 6})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
