#!/usr/bin/env python3
"""
TinyTrack - Generation 4
Turbo Evolved | Domain: daycare
"""
...
app = Flask(__name__)
CORS(app)
SECRET_KEY = 'swarm-daycare-secret-gen4'
...
def health():
    return jsonify({"status": "healthy", "app": "TinyTrack", "generation": 4, "timestamp": datetime.now().isoformat()})
...
def create_child():
    try:
        ...
        with get_db() as db:
            ...
            db.commit()
        return jsonify({"message": "Child created successfully"})
    except Exception as e:
        return jsonify({"error": str(e)}), 400

@app.route("/api/children/<int:item_id>", methods=["GET"])
@token_required
def get_child(item_id):
    try:
        with get_db() as db:
...
@token_required
def update_child(item_id):
    try:
        data = request.get_json()
        with get_db() as db:
            db.execute(
                "UPDATE children SET name=?, age=?, parent=?, phone=?, allergies=?, schedule=?, updated_at=? WHERE id=?",
...
def search_children():
    try:
        ...
        with get_db() as db:
            rows = db.execute(
                "SELECT * FROM children WHERE status LIKE ? ORDER BY created_at DESC LIMIT 50",
                (f"%{q}%",)
            ).fetchall()
        return jsonify([dict(r) for r in rows])
    except Exception as e:
...
@app.route("/api/v1/children", methods=["GET"])
@token_required
def get_children_v1():
    ...
    try:
        ...
        offset = (page - 1) * limit
        with get_db() as db:
            total = db.execute("SELECT COUNT(*) as count FROM children").fetchone()["count"]
            rows = db.execute("SELECT * FROM children ORDER BY created_at DESC LIMIT ? OFFSET ?", (limit, offset)).fetchall()
        return jsonify({
...
def stats():
    try:
        with get_db() as db:
            total = db.execute("SELECT COUNT(*) as count FROM children").fetchone()["count"]
            active = db.execute("SELECT COUNT(*) as count FROM children WHERE status='active'").fetchone()["count"]
            recent = db.execute("SELECT COUNT(*) as count FROM children WHERE date(created_at) = date('now')").fetchone()["count"]
        return jsonify({"total": total, "active": active, "today": recent, "domain": "daycare", "generation": 4})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
