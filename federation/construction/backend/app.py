...
app = Flask(__name__)
CORS(app)
SECRET_KEY = 'swarm-construction-secret'
...
def health():
    return jsonify({'status': 'healthy', 'app': 'BuildTrack', 'timestamp': datetime.now().isoformat()})
...
def create_projects():
    try:
        ...
        with get_db() as db:
            cursor = db.execute(
                "INSERT INTO projects (name, client, address, start, budget) VALUES (?, ?, ?, ?, ?)",
                (data.get('name'), data.get('client'), data.get('address'), data.get('start'), data.get('budget'),)
            )
            db.commit()
...
if __name__ == '__main__':
    init_db()
    print(f'Starting BuildTrack on http://localhost:5000')
