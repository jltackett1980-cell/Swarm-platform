#!/usr/bin/env python3
"""
BuildTrack - Generation 4
Turbo Evolved | Domain: construction
"""
...
app = Flask(__name__)
CORS(app)
SECRET_KEY = 'swarm-construction-secret-gen4'
...
def health():
    return jsonify({"status": "healthy", "app": "BuildTrack", "generation": 4, "timestamp": datetime.now().isoformat()})
...
def create_project():
    try:
        ...
        with get_db() as db:
            db.execute(
                "INSERT INTO projects (name, client, address, start, budget, status) VALUES (?, ?, ?, ?, ?, ?)",
...
@app.route("/api/projects/<int:item_id>", methods=["GET"])
@token_required
...
@app.route("/api/projects/<int:item_id>", methods=["PUT"])
@token_required
def update_project(item_id):
    try:
        ...
        with get_db() as db:
            db.execute(
                "UPDATE projects SET name=?, client=?, address=?, start=?, budget=?, status=?, updated_at=? WHERE id=?",
...
def search_projects():
    try:
        ...
        with get_db() as db:
            rows = db.execute(
                "SELECT * FROM projects WHERE status LIKE ? ORDER BY created_at DESC LIMIT 50",
                (f"%{q}%",)
            ).fetchall()
        return jsonify([dict(r) for r in rows])
    except Exception as e:
...
def get_projects_v1():
    ...
    try:
        ...
        offset = (page - 1) * limit
        with get_db() as db:
            total = db.execute("SELECT COUNT(*) as count FROM projects").fetchone()["count"]
            rows = db.execute("SELECT * FROM projects ORDER BY created_at DESC LIMIT ? OFFSET ?", (limit, offset)).fetchall()
        return jsonify({
...
def stats():
    try:
        with get_db() as db:
            total = db.execute("SELECT COUNT(*) as count FROM projects").fetchone()["count"]
            active = db.execute("SELECT COUNT(*) as count FROM projects WHERE status='active'").fetchone()["count"]
            recent = db.execute("SELECT COUNT(*) as count FROM projects WHERE date(created_at) = date('now')").fetchone()["count"]
        return jsonify({"total": total, "active": active, "today": recent, "domain": "construction", "generation": 4})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
