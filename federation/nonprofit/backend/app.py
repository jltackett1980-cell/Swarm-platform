...
app = Flask(__name__)
CORS(app)
SECRET_KEY = 'swarm-nonprofit-secret'
...
def health():
    return jsonify({'status': 'healthy', 'app': 'MissionBase', 'timestamp': datetime.now().isoformat()})
    ...
    try:
        ...
        with get_db() as db:
            cursor = db.execute(
                ...
                (data.get('name'), data.get('email'), data.get('phone'), data.get('amount'), data.get('date'), data.get('campaign'),)
            )
            db.commit()
...
def update_donors(item_id):
    try:
        data = request.get_json()
        sets = ", ".join([f"{k}=?" for k in data.keys()])
        vals = list(data.values()) + [item_id]
        with get_db() as db:
            db.execute(f"UPDATE donors SET {sets} WHERE id=?", vals)
            db.commit()
...
if __name__ == '__main__':
    init_db()
    print(f'Starting MissionBase on http://localhost:5000')
