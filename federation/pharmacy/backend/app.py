...
app = Flask(__name__)
CORS(app)
SECRET_KEY = 'swarm-pharmacy-secret-gen5'
...
def health():
    return jsonify({"status": "healthy", "app": "Pharmacy Manager", "generation": 5, "timestamp": datetime.now().isoformat()})
...
def get_prescriptions_v1():
    ...
    try:
        ...
        with get_db() as db:
            ...
            rows = db.execute("SELECT * FROM prescriptions ORDER BY created_at DESC LIMIT ? OFFSET ?", (limit, offset)).fetchall()
        return jsonify({
...
def stats():
    try:
        ...
        return jsonify({"total": total, "active": active, "today": recent, "domain": "pharmacy", "generation": 5})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
